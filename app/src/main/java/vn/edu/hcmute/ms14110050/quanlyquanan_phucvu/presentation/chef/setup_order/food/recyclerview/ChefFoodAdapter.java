package vn.edu.hcmute.ms14110050.quanlyquanan_phucvu.presentation.chef.setup_order.food.recyclerview;
...
import vn.edu.hcmute.ms14110050.quanlyquanan_phucvu.presentation.chef.setup_order.food.recyclerview.viewholder.ChefFoodVH;
...
public class ChefFoodAdapter extends BaseAdapter<ChefFoodVH, Food> {
    private IFoodVM containerVM;

    private final int VIEW_DATA = VIEW_EMPTY + 1;

    public ChefFoodAdapter(Activity activity) {
        super(activity);
    }
    ...
    @NonNull
    @Override
    public ChefFoodVH onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        LayoutInflater inflater = LayoutInflater.from(parent.getContext());

        if (viewType == VIEW_DATA) {
            ChefItemRecyclerFoodBinding binding =
                    DataBindingUtil.inflate(inflater, R.layout.chef_item_recycler_food, parent, false);
            return new ChefFoodVH(binding, containerVM);
        }
        else{
            ...
            return new ChefFoodVH(view);
        }
    }

    @Override
    public void onBindViewHolder(@NonNull ChefFoodVH holder, int position) {
        if (constainData()) {
